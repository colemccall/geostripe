...
- **926 unit tests** across geometry, curves, junction detection, intersection geometry,
...
| **Lanes along a street** | The cross-section varies with distance, so four lanes can run up to an interchange, one peel off into a ramp, and three carry on — one road, one alignment, three widths. The change tapers rather than stepping, and the lanes that continue keep their exact position: adding or dropping one moves only the edge, never the whole road |
