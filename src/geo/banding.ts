...
/**
 * Lane markings: the interior boundaries between two roadway components.
 *
 * Drawn separately from the bands so a divider reads as a line rather than as the seam
 * between two fills. Yellow where travel directions oppose, white otherwise — the same
 * rule real striping follows.
 */
export function markingsForStreet(
  streetId: string,
  centerline: readonly LngLat[],
  section: CrossSection,
): Feature[] {
  ...
  for (let i = 1; i < section.components.length; i++) {
    ...
    if (!PRIMITIVES[before.componentType].isRoadway) continue;
    if (!PRIMITIVES[after.componentType].isRoadway) continue;

    const opposing =
      (before.direction === 'backward' && (after.direction === 'forward' || after.direction === 'both')) ||
      (before.direction === 'both' && after.direction === 'forward');

    const boundary = offsets[i];
    if (boundary === undefined) continue;

    const coords = offsetPolyline(planePts, -boundary).map(
    ...
    out.push({
      type: 'Feature',
      id: `${streetId}:mark:${i}`,
      properties: { featureClass: 'marking', streetId, opposing },
